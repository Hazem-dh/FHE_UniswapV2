...
/**
 * Deploys  the amm contracts using the deployer account and
 * constructor arguments set to the deployer address
 *
 * @param hre HardhatRuntimeEnvironment object.
 */
const deployAMM: DeployFunction = async function (
  ...
  */
  ...
  const factory = await deploy("CyfherFactory", {
    from: deployer,
  ...
  console.log("factory deployed with address : ", factory.address)
    const router = await deploy("CyfherRouter", {
        from: deployer,
        args: [factory.address],
        log: true,
        autoMine: true,
    });
    console.log("router deployed with address : ", router.address)
